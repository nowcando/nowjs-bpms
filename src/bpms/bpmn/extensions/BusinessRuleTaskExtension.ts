...
function BusinessRuleTaskService(activity: any) {
    ...
    function execute(executionMessage: any, callback: any) {
      ...
      if (typeof serviceFn !== "function") { return callback(new Error(`${behaviour.implementation ? "Implementation" : "Expression" }  did not resolve to a function`)); }
...
export const BusinessRuleTaskExtension = (processInstance: BpmnProcessInstance) =>
     (activity: any) => {
