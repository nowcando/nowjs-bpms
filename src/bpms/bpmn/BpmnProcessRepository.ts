...
export interface BpmnProcessRepository {
    ...
    list<R extends BpmnProcessPersistedData>(options?: BpmnProcessListOptions): Promise<R[]>;
    find<R extends BpmnProcessPersistedData>(options: BpmnProcessFindOptions): Promise<R[]>;
    load<R extends BpmnProcessPersistedData>(options: BpmnProcessLoadOptions): Promise<R[]>;
...
export class BpmnProcessMemoryRepository implements BpmnProcessRepository {
    ...
    public async remove(options: BpmnProcessRemoveOptions): Promise<boolean> {
        const f = this.store.findIndex(
            xx => (options.name && xx.name === options.name) || (options.id && xx.id === options.id),
        );
        if (f >= 0) {
    ...
    public list<R extends BpmnProcessPersistedData>(options?: BpmnProcessListOptions | undefined): Promise<R[]> {
        const f =
            options && (options.id || options.name || options.state)
                ? this.store.filter(xx => (xx.id && xx.id === options.id) || (xx.name && xx.name === options.name))
                : this.store.slice();
        return Promise.resolve(f as any);
    }
    public find<R extends BpmnProcessPersistedData>(options: BpmnProcessFindOptions): Promise<R[]> {
        const f = this.store.filter(xx => (xx.id && xx.id === options.id) || (xx.name && xx.name === options.name));
        return Promise.resolve(f as any);
    }
    public load<R extends BpmnProcessPersistedData>(options: BpmnProcessLoadOptions): Promise<R[]> {
        const f = this.store.filter(xx => (xx.id && xx.id === options.id) || (xx.name && xx.name === options.name));
        const d = f.map(xx => {
            return {
                ...xx,
    ...
    public persist(options: BpmnProcessPersistOptions): Promise<boolean> {
        const ix = this.store.findIndex(xx => xx.id === options.id || xx.name === options.name);
        if (ix >= 0) {
