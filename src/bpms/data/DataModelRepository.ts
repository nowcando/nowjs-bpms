...
export interface DataModelRepository {
    ...
    list<R extends DataModelPersistedData>(options?: DataModelListOptions): Promise<R[]>;
    find<R extends DataModelPersistedData>(options: DataModelFindOptions): Promise<R | null>;
    load<R extends DataModelPersistedData>(options: DataModelLoadOptions): Promise<R[]>;
...
export class DataModelMemoryRepository implements DataModelRepository {
    ...
    public async list<R extends DataModelPersistedData>(options?: DataModelListOptions | undefined): Promise<R[]> {
        if (options) {
    ...
    public async find<R extends DataModelPersistedData>(options: DataModelFindOptions): Promise<R | null> {
        if (options) {
    ...
    public async load<R extends DataModelPersistedData>(options: DataModelLoadOptions): Promise<R[]> {
        if (options) {
            const f = this.store.filter(xx => xx.name === options.name);
            return f as any;
        } else {
