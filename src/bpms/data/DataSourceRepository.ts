...
export interface DataSourceRepository {
    ...
    list<R extends DataSourcePersistedData>(options?: DataSourceListOptions): Promise<R[]>;
    find<R extends DataSourcePersistedData>(options: DataSourceFindOptions): Promise<R | null>;
    load<R extends DataSourcePersistedData>(options: DataSourceLoadOptions): Promise<R[]>;
...
export class DataSourceMemoryRepository implements DataSourceRepository {
    ...
    public async list<R extends DataSourcePersistedData>(options?: DataSourceListOptions | undefined): Promise<R[]> {
        if (options) {
    ...
    public async find<R extends DataSourcePersistedData>(options: DataSourceFindOptions): Promise<R | null> {
        if (options) {
    ...
    public async load<R extends DataSourcePersistedData>(options: DataSourceLoadOptions): Promise<R[]> {
        if (options) {
            const f = this.store.filter(xx => xx.name === options.name);
            return f as any;
        } else {
