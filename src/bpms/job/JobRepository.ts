...
export class JobMemoryRepository<T extends Job = Job>
  implements JobRepository<T> {
  private tasks: T[] = [];
  ...
  public async updateJobFollowUpDate(
    taskId: string,
    followUpDate: Date | undefined,
  ): Promise<T> {
    const t = this.tasks.find((xx) => xx.id === taskId);
  ...
  public async updateJobCategories(
    taskId: string,
    categories: string | undefined,
  ): Promise<T> {
    const t = this.tasks.find((xx) => xx.id === taskId);
  ...
  public async countJobs(
    options?: JobQueryFilter | undefined,
  ): Promise<number> {
